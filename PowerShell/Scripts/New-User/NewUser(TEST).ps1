...
foreach ($user in $userData) {
    ...
    # Retrieve the Manager's DistinguishedName from the CSV file (make sure you have a "Manager" column)
    ...
    $newUser = New-ADUser -Name $fullName -GivenName $firstName -Surname $lastName -SamAccountName $logonName -UserPrincipalName "$logonName@epsilonsystems.com" -AccountPassword (ConvertTo-SecureString $password -AsPlainText -Force) -Enabled $true -Path $ouPath -Description $description -Office $office -EmailAddress "$logonName@epsilonsystems.com" -DisplayName $fullName -Manager $managerDN
